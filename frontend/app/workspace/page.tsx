...
export default function WorkspacePage() {
  ...
  const handleLoadCyberSample = async () => {
    let sample = {
      ...
      content: `INCIDENT POST-MORTEM & TECHNICAL INVESTIGATION REPORT
...
4. Schedule independent third-party penetration testing and source code verification for Q2 2026.`
    ...
    try {
      const remoteSample = await fetchCybersecuritySample();
      if (remoteSample && remoteSample.content) {
        sample = remoteSample;
      }
    } catch (e) {
      console.warn("Using embedded sample fallback:", e);
    }
